...
def process(operation_mode, source_files, out_path):
    ...
    # #########################################################
    if operation_mode == 'workflow_legacy':
    ...
    # # #################### workflow_go #####№################
    # # Рабочий режим обработки изображения с созданием выходной маски.
    # # Использует модели в формате onnx
    # # #########################################################
    # if operation_mode == 'workflow_':
    #     # TODO: импорт модуля для работы в формате onnx
    #     import sam2_model_onnx
    #
    #
    #
    #     model_onnx = sam2_model_onnx.get_model_sam2_onnx(encoder_file=s.SAM2_onnx_encoder,
    #                                                      decoder_file=s.SAM2_onnx_decoder,
    #                                                      verbose=s.VERBOSE)
    ...
    # Использует модели в формате onnx
    # #########################################################
    if operation_mode == 'workflow_hybrid':
        # Загружаем только изображения
        img_file_list = u.get_files_by_type(source_files, s.ALLOWED_IMAGES)
        if len(img_file_list) < 1:
            print("Не нашли изображений для обработки")
        ...
        Tool_list.append(t.Tool('model_sam2_onnx',
                            sam2_model_onnx.get_model_sam2_onnx(encoder_file=s.SAM2_onnx_encoder,
                                                                decoder_file=s.SAM2_onnx_decoder,
                                                                verbose=s.VERBOSE),
                            tool_type='model'))
        ...
        #
        tool_model_sam2 = t.get_tool_by_name('model_sam2', tool_list=Tool_list)
        tool_model_sam2_onnx = t.get_tool_by_name('model_sam2_onnx', tool_list=Tool_list)
        ...
        for img_idx, img in enumerate(img_list):
            ...
            # Ресайз к оригинальному разрешению и сохранение маски, полученной в разрешении 1024
            result_mask1024 = w.convert_mask_to_image(combined_mask)
            result_mask1024_original_size = cv.resize(result_mask1024, (W, H), interpolation=cv.INTER_LANCZOS4)  # cv.INTER_CUBIC
            ...
            # ##############################################################
            # Инициализация предиктора PyTorch
            predictor = sam2_model.get_predictor(tool_model_sam2.model, verbose=s.VERBOSE)
            if predictor is not None:
                print("Предиктор PyTorch инициализирован успешно")
            # ##############################################################
            ...
            for idx, center_original in enumerate(center_of_mass_original_list[:]):
                ...
                # Берем изображение фрагмента текстуры вокруг центра
                image_center = image_rgb_original[Y1:Y2, X1:X2].copy()
                # print("image_center.shape", image_center.shape)
                # u.show_image_cv(image_center, title=str(image_center.shape))

                # ##############################################################
                # Предикт в формате PyTorch
                # ##############################################################
                predictor.set_image(image_center)  # заносим изображение в модель
                ...
                # ##############################################################
                # TODO: Предикт в формате onnx
                # ##############################################################
                print("Расчет эмбеддингов изображения моделью onnx")
                embedding = tool_model_sam2_onnx.model.encode(image_center.copy())

                promt = [{"type": "point", "data": [Xp, Yp], "label": 1}]

                masks_onnx = tool_model_sam2_onnx.model.predict_masks(embedding, promt)

                # Merge masks
                mask_onnx = np.zeros((masks_onnx.shape[2], masks_onnx.shape[3], 3), dtype=np.uint8)
                for m in masks_onnx[0, :, :, :]:
                    mask_onnx[m > 0.5] = [255, 0, 0]
                #
                # Binding image and mask
                visualized = cv.addWeighted(image_center, 0.5, mask_onnx, 0.5, 0)
                # u.show_image_cv(visualized, title=f"{visualized.shape}")
