...
Используются оригинальные модели в формате onnx
"""
# import numpy as np
# import cv2 as cv
import time
...
from sam2onnx.sam2onnx import SegmentAnything2ONNX

# import settings as s

# #########################################################
# Функции для работы с моделью SAM2 в формате onnx
# #########################################################
def get_model_sam2_onnx(encoder_file,
                        decoder_file,
                        # force_cuda=False, TODO: не используется
                        verbose=False):
    """
    Загрузка модели из указанного файла onnx

    :param encoder_file: путь к файлу энкодера
    :param decoder_file: путь к файлу декодера
    :param force_cuda: использовать CUDA
    :param verbose: выводить дополнительную информацию
    :return: загруженная модель
    """
    if verbose:
        print("  Загружаем файл энкодера модели: {}".format(encoder_file))
        print("  Загружаем файл декодера модели: {}".format(decoder_file))
    time_0 = time.perf_counter()

    model_onnx = SegmentAnything2ONNX(encoder_model_path=encoder_file,  # TODO: убрать варнинги
                                      decoder_model_path=decoder_file)
...
# def get_predictor_onnx(model, verbose=False):
#     """
#     Инициализация предиктора из модели onnx
#
#     :param model: модель
#     :param verbose: выводить дополнительную информацию
